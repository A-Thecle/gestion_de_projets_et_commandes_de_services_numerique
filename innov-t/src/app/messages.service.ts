...
export class MessagesService {
  ...
  private sujetMessageSupprime = new Subject<{ id: string, scope: string, userId?: number }>();
surMessageSupprime(): Observable<{ id: string, scope: string, userId?: number }> {
  return this.sujetMessageSupprime.asObservable();
}
  ...
  private connecterSocket(token: string): void {
    try {
      ...
      this.socket.on('messageDeleted', (payload: { id: string, scope: string, userId?: number }) => {
  console.log('messageDeleted reçu:', payload);
  // on l'émet au component via sujet dédié
  this.sujetMessageSupprime.next(payload);
});
...
// dans messages.service.ts (frontend)
supprimerMessage(idMessage: string, scope: 'me' | 'all'): Observable<any> {
  return this.http.delete(`${this.apiUrl}/${idMessage}?scope=${scope}`, { headers: this.getAuthHeaders() })
    .pipe(
      catchError((err: HttpErrorResponse) => {
        if (err.status === 401) { this.authService.logout(); this.router.navigate(['/connexion']); }
        return throwError(() => new Error(`Erreur ${err.status}: ${err.message}`));
      })
    );
}
